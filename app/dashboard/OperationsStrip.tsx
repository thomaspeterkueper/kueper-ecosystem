...
type Operations = { available:boolean; generated_at:string; reason?:string; workers:Worker[]; queue:Record<string,number>; providers:Record<string,string>; llm_budget?:Record<string,Budget>; blocked_tasks?:number };
...
export default function OperationsStrip() {
  ...
  const waiting=(data?.queue?.pending||0)+(data?.queue?.review_pending||0); const deepseek=data?.providers?.deepseek||"unknown"; const budget=data?.llm_budget?.deepseek;
