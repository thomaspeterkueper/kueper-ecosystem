...
type UsageSource = { source_system:string; planned:number; executed:number; completed:number; failed:number; blocked:number; skipped:number; unreconciled_calls:number; estimated_cost_usd:number|string|null; actual_cost_usd:number|string|null };
type AIUsage = { planned:number; executed:number; completed:number; failed:number; blocked:number; skipped:number; deduped:number; unreconciled_calls:number; input_tokens:number; output_tokens:number; estimated_cost_usd:number|string|null; actual_cost_usd:number|string|null; by_source:UsageSource[]; latest:any[] };
...
function usd(value:number|string|null|undefined) { if(value===null||value===undefined)return "nicht reconciled"; const n=Number(value); return `$${n.toFixed(n<1?3:2)}`; }
...
export default function OperationsStrip() {
  ...
  return <section className={styles.shell}>
    ...
    <div className={styles.grid}>
      ...
      <article className={styles.card}><div className={styles.cardTop}><span>AI Usage heute</span><span className={`${styles.state} ${usage&&usage.failed?styles.warning:styles.ok}`}>{usage?`${usage.executed}/${usage.planned} ausgeführt`:"ledger"}</span></div><div className={styles.value}>{usage?usd(usage.actual_cost_usd):"—"}</div><div className={styles.caption}>{usage?`${usage.completed} fertig · ${usage.blocked} blockiert · ${usage.deduped} dedupliziert · ${usage.unreconciled_calls} Kosten offen`:data.ai_usage_reason||"Ledger wird nach DB-Migration verfügbar"}</div><a className={styles.ledgerLink} href="/dashboard/ai-usage">Ledger öffnen →</a></article>
      <article className={styles.card}><div className={styles.cardTop}><span>Top AI-Verbraucher</span><span className={`${styles.state} ${styles.neutral}`}>heute</span></div><div className={styles.value}>{topSource?.source_system||"—"}</div><div className={styles.caption}>{topSource?`${topSource.executed}/${topSource.planned} Calls · ${usd(topSource.actual_cost_usd)} · ${topSource.unreconciled_calls} unreconciled`:"noch keine Nutzungsdaten"}</div></article>
