...
-- Repo-history copy of the already deployed control-plane migration.
-- Activation is explicit; this file does not enable cron jobs by itself.
...
create index if not exists scheduler_runs_worker_created_idx on ecosystem.scheduler_runs(worker_name, created_at desc);
create index if not exists scheduler_runs_status_created_idx on ecosystem.scheduler_runs(status, created_at desc);
...
returns trigger language plpgsql set search_path=ecosystem,pg_temp as $$
begin new.updated_at := now(); return new; end $$;

drop trigger if exists scheduler_runs_touch_updated_at on ecosystem.scheduler_runs;
create trigger scheduler_runs_touch_updated_at before update on ecosystem.scheduler_runs
...
language plpgsql security definer set search_path=ecosystem,public,pg_temp as $$
declare
...
begin
  if p_worker_name not in ('agent-worker-v7','pr-review-agent') then raise exception 'unsupported scheduler worker: %', p_worker_name; end if;
  if p_source not in ('supabase','github_schedule','manual') then raise exception 'invalid scheduler source: %', p_source; end if;
  if p_lease_seconds < 60 or p_lease_seconds > 7200 then raise exception 'lease seconds out of range'; end if;
  if p_cooldown_seconds < 0 or p_cooldown_seconds > 1800 then raise exception 'cooldown seconds out of range'; end if;

  if p_run_id is not null then
    select * into v_run from ecosystem.scheduler_runs where id = p_run_id and worker_name = p_worker_name for update;
    if v_run.id is null then raise exception 'scheduler run not found for worker'; end if;
  else
    insert into ecosystem.scheduler_runs(worker_name, source, scheduled_for, status)
    values (p_worker_name, p_source, v_now, 'planned') returning * into v_run;
  end if;

  insert into ecosystem.scheduler_leases(worker_name) values (p_worker_name) on conflict (worker_name) do nothing;
  select * into v_lease from ecosystem.scheduler_leases where worker_name = p_worker_name for update;

  if v_lease.lease_expires_at is not null and v_lease.lease_expires_at > v_now then
    update ecosystem.scheduler_runs set status='skipped', finished_at=v_now, last_error='active execution lease' where id=v_run.id;
    return jsonb_build_object('acquired',false,'run_id',v_run.id,'reason','active_lease');
  end if;

  if v_lease.last_finished_at is not null and v_lease.last_finished_at > v_now - make_interval(secs => p_cooldown_seconds) then
    update ecosystem.scheduler_runs set status='skipped', finished_at=v_now, last_error='recent execution cooldown' where id=v_run.id;
    return jsonb_build_object('acquired',false,'run_id',v_run.id,'reason','cooldown');
  end if;

  update ecosystem.scheduler_leases
     set run_id=v_run.id, lease_token=v_token,
...
language plpgsql security definer set search_path=ecosystem,public,pg_temp as $$
declare v_worker text; v_now timestamptz := now();
begin
  if p_status not in ('succeeded','failed') then raise exception 'invalid terminal scheduler status'; end if;
  select worker_name into v_worker from ecosystem.scheduler_runs where id=p_run_id and lease_token=p_lease_token for update;
  if v_worker is null then raise exception 'scheduler run lease invalid'; end if;
  update ecosystem.scheduler_runs
     set status=p_status, finished_at=v_now, github_run_id=coalesce(p_github_run_id,github_run_id), last_error=p_error
   where id=p_run_id;
  update ecosystem.scheduler_leases
     set run_id=null, lease_token=null, lease_expires_at=null, last_finished_at=v_now, updated_at=v_now
...
returns uuid
language plpgsql security definer set search_path=ecosystem,public,extensions,vault,pg_temp as $$
declare
...
begin
  case p_worker_name
    ...
    else raise exception 'unsupported scheduler worker: %', p_worker_name;
  end case;

  select decrypted_secret into v_token from vault.decrypted_secrets where name='kueper_github_dispatch_token' limit 1;
  if coalesce(v_token,'')='' then raise exception 'Vault secret kueper_github_dispatch_token is not configured'; end if;
...
exception when others then
  if exists(select 1 from ecosystem.scheduler_runs where id=v_run_id) then
    update ecosystem.scheduler_runs set status='failed', finished_at=now(), last_error=sqlerrm where id=v_run_id;
  end if;
  raise;
end $$;
...
returns jsonb
language plpgsql security definer set search_path=ecosystem,public,extensions,vault,cron,pg_temp as $$
declare v_token text;
begin
  select decrypted_secret into v_token from vault.decrypted_secrets where name='kueper_github_dispatch_token' limit 1;
  if coalesce(v_token,'')='' then raise exception 'Vault secret kueper_github_dispatch_token is not configured'; end if;
  perform cron.unschedule(jobid) from cron.job where jobname in ('kueper-external-agent-worker-v7','kueper-external-pr-review-agent');
  perform cron.schedule('kueper-external-agent-worker-v7','*/15 * * * *',$cron$select public.kueper_scheduler_dispatch('agent-worker-v7');$cron$);
  perform cron.schedule('kueper-external-pr-review-agent','7,22,37,52 * * * *',$cron$select public.kueper_scheduler_dispatch('pr-review-agent');$cron$);
...
returns jsonb
language plpgsql security definer set search_path=ecosystem,public,extensions,cron,pg_temp as $$
begin
  perform cron.unschedule(jobid) from cron.job where jobname in ('kueper-external-agent-worker-v7','kueper-external-pr-review-agent');
...
returns table(worker_name text,last_dispatch_at timestamptz,last_started_at timestamptz,last_finished_at timestamptz,last_status text,stale boolean)
language sql security definer set search_path=ecosystem,public,pg_temp as $$
  with workers(worker_name,max_age) as (
    values ('agent-worker-v7'::text, interval '25 minutes'), ('pr-review-agent'::text, interval '25 minutes')
  ), latest as (
    select distinct on (r.worker_name)
           r.worker_name,r.dispatch_requested_at,r.started_at,r.finished_at,r.status,r.created_at
      from ecosystem.scheduler_runs r order by r.worker_name,r.created_at desc
  )
  select w.worker_name,l.dispatch_requested_at,l.started_at,l.finished_at,l.status,
         (l.created_at is null or l.created_at < now()-w.max_age) as stale
    from workers w left join latest l using(worker_name) order by w.worker_name;
$$;
